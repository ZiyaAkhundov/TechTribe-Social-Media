    ...
    <div className={`navbar bg-bgMain h-14 flex px-3 md:z-50 items-center ${user ? 'justify-between' : 'justify-between lg:justify-end'}`}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
        <Tooltip title="Account settings">
          ...
          >
            ...
            <Avatar sx={{ width: 32, height: 32 }} src={user.picture ? user.picture.url : null} className="border"></Avatar>
          </IconButton>
        </Tooltip>
      </Box>
      <Menu
      ...
      >
        <MenuItem onClick={handleClose}>
          <a href={`../profile/${user.username}`} className='flex items-center'>
          <Avatar src={user.picture ? user.picture.url : null} className="border"/> Profile
          </a>
        </MenuItem>
        <Divider />
        <MenuItem onClick={handleClose}>
